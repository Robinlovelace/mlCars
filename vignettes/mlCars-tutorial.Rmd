...
Convert to numeric
```{r}
flows = mutate_all(wyflows@data, as.numeric)
```
...
plot(wyflows@data$metro, flows$train)
plot(wyflows@data$metro, xg_fitted, col = "red")
```

Calculate how well the model has performed
```{r}
xgb1 = sqrt(c(crossprod(xg_fitted - wyflows@data$metro)) / nrow(flows))
```
