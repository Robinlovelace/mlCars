...
Build a few linear models with car as the dependent variable
```{r}
mAge <- lm(car ~ q1624 + q2534 + q2534 + q3549 + q5064 + q6574 + q75plus, data =df.wyflows)
summary(m0)
```
...
```{r}
mHealth <- lm(car ~ vghealth + ghealth + fhealth + bhealth + vbhealth + q1624 + q2534 + q3549 + q5064 + q6574 + q75plus, data=df.wyflows)
summary(mHealth0)
```
...
### Machine Learning (still needs quite a bit of work)
**Idea: explore option of using proportions**
...
Convert to numeric
```{r}
flows = mutate_all(df.wyflows, as.numeric)
```

Build a model, focusing on metro transport. In the analysis we focused on the car, however
```{r}
xg = xgboost(data = as.matrix(flows), label = wyflows$metro, nrounds = 10,  max_depth = 7)
```

Make a prediction
```{r}
xg_fitted = predict(xg, as.matrix(flows))
```

Plot the metro against the train and the fitted points
```{r, fig.keep='all', out.width="50%"}
plot(df.wyflows$metro, flows$train)
plot(df.wyflows$metro, xg_fitted, col = "red")
```

Calculate how well the model has performed
```{r}
xgb1 = sqrt(c(crossprod(xg_fitted - df.wyflows$metro)) / nrow(flows))
```

Plot the variable importance
```{r}
importance_xgb = xgb.importance(model = xg, feature_names = names(flows))
