...
There are many possible ways of defining and measuring car use. In this work as a measure of car use we have chosen to use the fraction of people (commuting between two particular geographical zones) which stated in the Census that they either drive, or are passengers in a car or van.
...
```{r variablestable, echo=F, eval=T}
...
The model which best fit how the car use fraction varies with distance turned out to be a logistic regression model fit on the mean car fraction by 1km distance band (i.e. a curve where the first point is the average car use fraction for distances of 0-1km, etc.) using the distance and the natural logarithm of the distance as input variables. The figure below shows the fit obtained.
...
After subtractin the car fraction predicted by the model from the observed car fraction we obtain a variable which a much reduced dependence on distance (i.e. it is centred on zero and does not significantly increase or decrease with distance), as shown in the figure below.
